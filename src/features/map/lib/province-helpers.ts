import { findCountryByTag, findProvinceById, findRegionByProvinceId, MOCK_MAP_DATA } from '../data/mock-world';
...
export const getProvinceSummary = (provinceId: string | null): ProvinceSummaryViewModel | null => {
	if (!provinceId) {
		return null;
	}
	...
	const owner = findCountryByTag(province.ownerTag) ??
		MOCK_MAP_DATA.countries.find((country) => country.tag === province.ownerTag);
	const controller = findCountryByTag(province.controllerTag) ??
		MOCK_MAP_DATA.countries.find((country) => country.tag === province.controllerTag);
